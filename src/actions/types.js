export const SET_PAGE = 'SET_PAGE';
export const SET_POPUP = 'SET_POPUP';
export const SET_GEWUERFELTE_ZAHL = 'SET_GEWUERFELTE_ZAHL';
export const SET_CLIENT_ID = 'SET_CLIENT_ID';
export const SET_SPIEL_ID = 'SET_SPIEL_ID';
export const SET_SPIELFELD_ARRAY = 'SET_SPIELFELD_ARRAY';
export const SET_FRAGE = 'SET_FRAGE';
export const SET_CLIENTS = 'SET_CLIENTS';
export const SET_WER_IST_DRAN = 'SET_WER_IST_DRAN';
export const SET_SPIELFIGUR_POSITIONEN = 'SET_SPIELFIGUR_POSITIONEN';
export const RESET_SPIELFIGUR_POSITIONEN = 'RESET_SPIELFIGUR_POSITIONEN';
export const SET_AKTION = 'SET_AKTION';
export const SET_STARTSEITE_LOG = 'SET_STARTSEITE_LOG';
export const SET_ANTWORT_FEEDBACK = 'SET_ANTWORT_FEEDBACK';